...
```
## Dataset 
Two files have been shared in the [dataset folder](datasets/armed_people_detection). The file named Videos1_2_3.csv was used to train the armed person detection models. The video_test.csv file was used as the test dataset. These datasets have been developed with the [videos](videos) that are shared in the repository. These have been processed by the [dataset generator](dataset_generator) algorithm. The ground truth was placed manually for each record of the dataset, verifying the possession of the weapons in each frame of the video.
